...
function App() {
  ...
  // clamp size to: >= MIN, <= natural size, and also not exceed canvas given fixed center
  const clampSizeWithAspect = (center, size, nat) => {
    // cap by natural size (no upscaling)
    ...
    // cap by canvas available space (center fixed)
    const rect = getCanvasRect()
    if (rect) {
  ...
  // keep aspect ratio on any size change
  ...
  const handleDrop = async (e) => {
    ...
    // read natural size so we can lock aspect and avoid upscaling
    const nat = await readNaturalSize(draggedImage.src)
    const aspect = nat.w / nat.h

    // initial base size (no bigger than natural)
    ...
    const newImage = {
      ...
      size: { w: initW, h: initH },     // matches aspect
      aspect,
      nat,                               // natural size
  ...
  const onResizeMove = (e) => {
    ...
    setDroppedImages((prev) =>
      prev.map((img) => {
        ...
        if (s.type === 'corner') {
          // Use distance from center to pointer => half-size; keep aspect
          const halfW = Math.abs(px - center.x)
          let next = keepAspectFromWidth(Math.max(MIN_W, Math.round(halfW * 2)), s.aspect)
          // clamp with natural + canvas
          next = clampSizeWithAspect(center, next, s.nat)
          w = next.w; h = next.h
        } else {
          // side resize: change one axis, keep aspect by adjusting the other
          if (s.edge === 'e' || s.edge === 'w') {
            // width from pointer (center-fixed)
            const halfW = Math.abs(px - center.x)
            let next = keepAspectFromWidth(Math.max(MIN_W, Math.round(halfW * 2)), s.aspect)
            next = clampSizeWithAspect(center, next, s.nat)
            w = next.w; h = next.h
            ...
            >
              ...
              {droppedImages.map((image, idx) => (
                <div
                  ...
                  // NOTE: no background, no shadow, no ring
                ...
                >
                  {/* Image fills exactly (no crop, no zoom, no extra background) */}
                  <img
                    src={image.src}
                    alt={image.name}
