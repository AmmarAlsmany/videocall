...
function App() {
  ...
  // keep the entire box inside canvas when center is fixed
  ...
  // when center is fixed, clamp size to stay inside
  ...
  const clampPointerForEdge = (edge, oppositeConst, desired, aspectData) => {
    const rect = getCanvasRect()
    if (!rect) return desired
    // min distance from opposite edge to pointer
    if (edge === 'e') {
  ...
  const onResizeMove = (e) => {
    ...
    setDroppedImages((prev) => {
      return prev.map((img) => {
        ...
        // default new size/center starts from existing
        ...
        if (session.type === 'corner') {
          // center-fixed unless Alt (anchorOpposite)
          if (!session.anchorOpposite) {
            // distance from center to pointer defines half sizes
            ...
            // anchor opposite corner: compute opposite corner from center/start size
            ...
            // clamp pointer to canvas bounds
            ...
            if (session.keepRatio) {
              // fit with aspect using whichever is limiting
            ...
            // center is midpoint between pointer and opposite corner
            center = { x: (clampedX + oppX) / 2, y: (clampedY + oppY) / 2 }

            // if overflow because of rounding, clamp center to keep inside
            center = clampCenter(center, { w: w, h: h })
          }
        } else if (session.type === 'side') {
          const edge = session.edge
          if (!session.anchorOpposite) {
            // center-fixed side resizing: adjust only one axis symmetrically
            if (edge === 'e' || edge === 'w') {
              let halfW = Math.abs(px - center.x)
              if (session.keepRatio) {
                // adjust height from width to keep ratio
            ...
            // anchor opposite edge: that edge stays fixed; center shifts
            if (edge === 'e') {
